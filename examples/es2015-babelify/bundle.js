...
_loaderModule.default.track('Tracking after alooma.init');
...
/** @const */
,
    UNION_ACTION = "$union" // This key is deprecated, but we want to check for it to see whether aliasing is allowed.
...
DomTracker.prototype.track = function (query, event_name, properties, user_callback) {
  ...
  _.each(elements, function (element) {
    _.register_event(element, this.override_event, function (e) {
      ...
      that.event_handler(e, this, options); // in case the alooma servers don't get back to us in time
...
var create_mplib = function create_mplib(token, config, name) {
  ...
  DEBUG = DEBUG || instance.get_config('debug'); // if target is not defined, we called init after the lib already
...
var AloomaLib = function AloomaLib() {}; // Initialization methods
...
AloomaLib.prototype.track_custom_event = function (event_object, callback) {
  ...
  properties['token'] = this.get_config('token'); // update persistence
...
AloomaPeople.prototype.delete_user = function () {
  ...
  var data = {
    '$delete': this._alooma.get_distinct_id()
  };
...
AloomaPeople.prototype._send_request = function (data, callback) {
  ...
  this._alooma._send_request(this._get_config('api_host') + '/engage/', {
    'data': encoded_data
...
MPNotif.prototype._init_video = _.safewrap(function () {
  ...
  self.yt_custom = 'postMessage' in window; // detect CSS compatibility
...
MPNotif.prototype._mark_delivery = _.safewrap(function (extra_props) {
  if (!this.marked_as_shown) {
    ...
    this._track_event('$campaign_delivery', extra_props); // mark notification shown (alooma property)
...
_['info']['properties'] = _.info.properties; // AloomaLib Exports
...
AloomaLib.prototype['_show_notification'] = AloomaLib.prototype._show_notification; // AloomaPersistence Exports
...
AloomaPersistence.prototype['clear'] = AloomaPersistence.prototype.clear; // AloomaPeople Exports
...
var add_dom_loaded_handler = function add_dom_loaded_handler() {
  ...
  if (document.addEventListener) {
    ...
    // IE
    document.attachEvent("onreadystatechange", dom_loaded_handler); // check to make sure we arn't in a frame
...
function init_from_snippet() {
  ...
  alooma_master = window[PRIMARY_INSTANCE_NAME]; // Initialization
  ...
  alooma_master['init'](); // Fire loaded events after updating the window's alooma object
